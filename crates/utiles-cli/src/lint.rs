...
fn check_metadata_for_duplicates(rows: Vec<MbtilesMetadataRow>) -> HashMap<String, Vec<MbtilesMetadataRow>> {
...
fn metadata2map(rows: MbtilesMetadataRows) -> UtilesLintResult<HashMap<String, String>> {
...
fn lint_mbtiles_file(fspath: PathBuf, fix: bool) {
    ...
    let mbtiles_result = utilesqlite::mbtiles::Mbtiles::from_filepath_str(
        fspath.to_str().unwrap()
    );
    ...
    let has_unique_index_on_metadata_name = mbtiles.has_unique_index_on_metadata().unwrap();
    ...
    if !map.contains_key("format") {
        ...
        if format_value != "png" && format_value != "jpg" && format_value != "pbf" && format_value != "webp" && format_value != "geojson" {
