...
// /// GeoJSON writer.
// pub struct GeoJsonBuilder<W: Write> {
//     // dims: CoordDimensions,
//     pub(crate) out: W,
// }

// impl<W: Write> GeoJsonBuilder<W> {
//     pub fn new(
//         // out: W
//     ) -> Self {
//         GeoJsonBuilder {
...
// impl<W: Write> FeatureProcessor for GeoJsonBuilder<W> {
...
// impl<W: Write> GeomProcessor for GeoJsonBuilder<W> {
...
// impl<W: Write> PropertyProcessor for GeoJsonBuilder<W> {
...
use geozero::{
    error::Result, ColumnValue, CoordDimensions, FeatureProcessor, GeomProcessor,
    PropertyProcessor,
};
use serde_json::Map;
use serde_json::Value as JsonValue;

/// A geometry "under construction".
#[derive(Debug)]
#[derive(Default)]
enum CurrentGeometry {
    #[default]
    None,
    /// e.g. "Point" or "LineString"
    /// We store the variant plus a nested structure of coordinates.
    Geometry {
        geom_type: &'static str, // "Point", "LineString", etc.
        coords: Vec<JsonValue>,  // We accumulate raw coordinate arrays in JSON form
    },
    /// For e.g. "MultiPoint", "MultiLineString", "Polygon", "MultiPolygon", etc.
    GeometryArray {
        geom_type: &'static str,
        coords_array: Vec<JsonValue>, // Each element is e.g. an array of coords
    },
    /// A "GeometryCollection" is a list of sub-geometries (which themselves can be nested).
    GeometryCollection {
...
/// Our builder that holds a `FeatureCollection` in memory.
pub struct GeoJsonBuilder {
    /// The final FeatureCollection we will build up
    pub collection: FeatureCollection,

    /// Currently active feature
    current_feature: Option<Feature>,

    /// Currently building geometry for the active feature
    current_geom: CurrentGeometry,

    /// Accumulate properties in a map until `properties_end()`
    current_props: Map<String, JsonValue>,

    /// A small stack to handle nested geometry states, e.g. geometry collections
    geom_stack: Vec<CurrentGeometry>,

    /// Keep track of dimension demands (XY vs XYZ).
...
impl GeoJsonBuilder {
    pub fn new() -> Self {
        GeoJsonBuilder {
            ...
            current_geom: CurrentGeometry::None,
            current_props: Map::new(),
            geom_stack: Vec::new(),
            dims: CoordDimensions::default(),
        }
    }
    fn finalize_geometry(&mut self) -> Result<()> {
        if let Some(feat) = &mut self.current_feature {
            // Take ownership of the geometry state:
            let geom_state = std::mem::take(&mut self.current_geom);
            // Convert it to an Option<Geometry>
            let geometry = GeoJsonBuilder::build_geometry(geom_state)?;
            feat.geometry = geometry;
        }
        Ok(())
    }
    /// Helper to finalize the geometry under construction and set it on the current feature.
    // fn finalize_geometry(&mut self) -> Result<()> {
    // if let Some(feat) = &mut self.current_feature {
    // Convert self.current_geom → geojson::Geometry
    // let geometry = self.build_geometry(std::mem::take(&mut self.current_geom))?;
    // feat.geometry = geometry;
    // }
    // Ok(())
    // }

    /// Recursively build a `geojson::Geometry` from our `CurrentGeometry` enum.
    fn build_geometry(geom: CurrentGeometry) -> Result<Option<Geometry>> {
        match geom {
            CurrentGeometry::None => Ok(None),

            CurrentGeometry::Geometry { geom_type, coords } => {
                // For e.g. "Point", "LineString"
                // coords might be one array or a nested array depending on type
                let geometry_value = match geom_type {
                    "Point" => {
                        // coords is a single array of [x,y,(z)]
                        if coords.len() != 1 {
                            // In a normal pipeline, we expect exactly 1 array
                            return Err(geozero::error::GeozeroError::Geometry(
                                "Invalid point coords".into(),
                            ));
                        }
                        let arr = coords[0]
                            .as_array()
                            .ok_or(geozero::error::GeozeroError::Geometry(
                                "Invalid point coords".into(),
                            ))?
                            .clone();
                        Value::Point(arr_f64(arr)?)
                    }
                    "LineString" => {
                        // coords is an array of points: e.g. [[x,y], [x,y], ...]
                        let mut line_coords = Vec::new();
                        for c in coords.iter() {
                            let arr = c
                                .as_array()
                                .ok_or(geozero::error::GeozeroError::Geometry(
                                    "Invalid line coords".into(),
                                ))?

                                .clone();
                            line_coords.push(arr_f64(arr)?);
                        }
                        Value::LineString(line_coords)
                    }
                    _ => {
                        return Err(
                            // format!("Unhandled geometry type {geom_type}").into()
                            geozero::error::GeozeroError::Geometry(
                                "Unhandled geometry type".into(),
                            ),
                        );
                    }
                };
                Ok(Some(Geometry::new(geometry_value)))
            }

            CurrentGeometry::GeometryArray {
                geom_type,
                coords_array,
            } => {
                // "MultiPoint", "MultiLineString", "Polygon", "MultiPolygon", etc.
                let geometry_value = match geom_type {
                    "MultiPoint" => {
                        // coords_array is e.g. array of [[x,y], [x,y], ...]
                        let mut mp_coords = Vec::new();
                        for c in coords_array.iter() {
                            // each c is a single coordinate array
                            let arr = c
                                .as_array()
                                .ok_or(geozero::error::GeozeroError::Geometry(
                                    "Invalid multipoint coords".into(),
                                ))?
                                .clone();
                            mp_coords.push(arr_f64(arr)?);
                        }
                        Value::MultiPoint(mp_coords)
                    }
                    "MultiLineString" => {
                        // coords_array is e.g. [ [[x,y],[x,y]], [[x,y],[x,y]] ]
                        // For each line, we have a sub-array of points
                        let mut mls_coords = Vec::new();
                        for line in coords_array.iter() {
                            let line_array = line.as_array().ok_or(
                                geozero::error::GeozeroError::Geometry(
                                    "Invalid multiline array".into(),
                                ),
                            )?;
                            ...
                            for pt in line_array {
                                let arr = pt
                                    .as_array()
                                    .ok_or(geozero::error::GeozeroError::Geometry(
                                        "Invalid multiline coord".into(),
                                    ))?
                                    .clone();
                                one_line.push(arr_f64(arr)?);
                            }
                            mls_coords.push(one_line);
                        }
                        Value::MultiLineString(mls_coords)
                    }
                    "Polygon" => {
                        // coords_array is e.g. [ [[x,y],[x,y]], [[x,y],[x,y]] ] (outer ring, holes, etc.)
                        // Each element is a ring
                        let mut rings = Vec::new();
                        for ring in coords_array {
                            let ring_array = ring.as_array().ok_or(
                                geozero::error::GeozeroError::Geometry(
                                    "Invalid polygon array".into(),
                                ),
                            )?;
                            let mut ring_coords = Vec::new();
                            for pt in ring_array {
                                let arr = pt
                                    .as_array()
                                    .ok_or(geozero::error::GeozeroError::Geometry(
                                        "Invalid polygon coord".into(),
                                    ))?
                                    .clone();
                                ring_coords.push(arr_f64(arr)?);
                            }
                    ...
                    "MultiPolygon" => {
                        // coords_array is e.g. [
                        //   [ [[x,y],[x,y]], [[x,y],[x,y]] ],   // polygon #1 (outer ring + holes)
                        //   [ [[x,y],[x,y]], [[x,y],[x,y]] ]    // polygon #2
                        // ]
                        let mut mp = Vec::new();
                        for poly in coords_array.iter() {
                            let poly_rings = poly.as_array().ok_or(
                                geozero::error::GeozeroError::Geometry(
                                    "Invalid multipolygon array".into(),
                                ),
                            )?;
                            let mut rings = Vec::new();
                            for ring in poly_rings {
                                let ring_array = ring.as_array().ok_or(
                                    geozero::error::GeozeroError::Geometry(
                                        "Invalid multipolygon ring".into(),
                                    ),
                                )?;
                                let mut ring_coords = Vec::new();
                                for pt in ring_array {
                                    let arr = pt
                                        .as_array()
                                        .ok_or(geozero::error::GeozeroError::Geometry(
                                            "Invalid multipolygon coord".into(),
                                        ))?
                                        .clone();
                                    ring_coords.push(arr_f64(arr)?);
                                }
                    ...
                    _ => {
                        return Err(
                            geozero::error::GeozeroError::Geometry(
                                "Unhandled geometry array type".into(),
                            ), // format!("Unhandled geometry array type {geom_type}").into()
                        );
                    }
                };
                Ok(Some(Geometry::new(geometry_value)))
            ...
            CurrentGeometry::GeometryCollection { geometries } => {
                // A geometry collection is a list of sub-Geometry
                Ok(Some(Geometry::new(Value::GeometryCollection(geometries))))
            }
        }
    }
}

/// Helper function to convert a `Vec<JsonValue>` (like `[x,y,z?]`) into a `Vec<f64>`.
fn arr_f64(arr: Vec<JsonValue>) -> Result<Vec<f64>> {
    let mut out = Vec::new();
    for v in arr {
...
impl GeomProcessor for GeoJsonBuilder {
    ...
    fn xy(&mut self, x: f64, y: f64, idx: usize) -> Result<()> {
        // If we're in a linestring, we push `[x, y]` into the active geometry's buffer.
        // If we're in a point, we either store it directly, etc.
        let coords = serde_json::json!([x, y]);
        self.push_coord(coords, idx)
    }
    ...
    fn coordinate(
        ...
        let coords = JsonValue::Array(arr.into_iter().map(JsonValue::from).collect());
        self.push_coord(coords, idx)
    }

    fn empty_point(&mut self, _idx: usize) -> Result<()> {
        // e.g. "Point" with an empty coordinates array
        self.current_geom = CurrentGeometry::Geometry {
            geom_type: "Point",
            coords: vec![JsonValue::Array(vec![])],
        };
        Ok(())
    }
    ...
    fn point_end(&mut self, _idx: usize) -> Result<()> {
        // nothing special: we rely on the calls to `xy(...)` or `coordinate(...)`
        Ok(())
    }

    fn multipoint_begin(&mut self, _size: usize, _idx: usize) -> Result<()> {
        self.current_geom = CurrentGeometry::GeometryArray {
            geom_type: "MultiPoint",
            coords_array: Vec::new(),
        };
        Ok(())
    }
    ...
    fn linestring_begin(
        ...
        if tagged {
            ...
            // We are inside e.g. polygon ring or multiline ring, so we just push a subarray
            if let CurrentGeometry::GeometryArray {
                geom_type,
                ref mut coords_array,
            } = self.current_geom
            {
                // e.g. "Polygon" or "MultiLineString"
                // Start a new subarray
                coords_array.push(JsonValue::Array(Vec::new()));
            }
        }
        Ok(())
    }
    ...
    fn linestring_end(&mut self, tagged: bool, _idx: usize) -> Result<()> {
        // If `tagged` is false, we are inside e.g. a polygon ring
        Ok(())
    }
    ...
    fn polygon_begin(&mut self, tagged: bool, _size: usize, _idx: usize) -> Result<()> {
        if tagged {
            ...
            // We might be inside multipolygon, so we add a polygon subarray
            if let CurrentGeometry::GeometryArray {
    ...
    fn geometrycollection_begin(&mut self, _size: usize, _idx: usize) -> Result<()> {
        // push the current_geom onto stack, start a new geometry collection
        self.geom_stack.push(std::mem::take(&mut self.current_geom));
        self.current_geom = CurrentGeometry::GeometryCollection {
            geometries: Vec::new(),
        };
        Ok(())
    }
    ...
    fn geometrycollection_end(&mut self, _idx: usize) -> Result<()> {
        // finalize the geometrycollection, pop the stack
        if let CurrentGeometry::GeometryCollection { geometries } =
            std::mem::take(&mut self.current_geom)
        {
            // build a geometry
            let geom = Geometry::new(Value::GeometryCollection(geometries));
            // put it back into the top of stack
            if let Some(mut parent) = self.geom_stack.pop() {
                match &mut parent {
                    CurrentGeometry::GeometryCollection { geometries } => {
                    ...
                    | CurrentGeometry::None => {
                        // If parent is a single geometry, we might need to wrap it, etc.
                        // We'll just do a naive approach: replace it with a geometry collection?
                        // Or fail. For simplicity, let's just fail if it doesn't match your structure.
                        return Err(geozero::error::GeozeroError::Geometry(
                            "Invalid geometry collection nesting".into(),
                        ));
                    }
                }
                self.current_geom = parent;
            }
        }
        Ok(())
    }
...
impl GeoJsonBuilder {
    /// Called by `xy(...)` or `coordinate(...)` to add one point to the current geometry.
    fn push_coord(&mut self, coord: JsonValue, _idx: usize) -> Result<()> {
        match &mut self.current_geom {
            CurrentGeometry::Geometry {
                geom_type: _,
                coords,
            } => {
            ...
            CurrentGeometry::GeometryArray {
                ...
                // If we're building e.g. "MultiPoint", coords_array is the top-level array
                // If we're in "Polygon", coords_array is the array of rings, but we need to push
                // into the last ring. So the last ring is coords_array.last_mut().
                if let Some(last) = coords_array.last_mut() {
                    if last.is_array() {
                        // We are in ring or line subarray
                        if let Some(arr) = last.as_array_mut() {
                            arr.push(coord);
                        }
                    } else {
                        // If it's "MultiPoint", we push directly at top level
                        coords_array.push(coord);
                    }
                } else {
                    // if "MultiPoint", we push directly
                    coords_array.push(coord);
                }
            }
            CurrentGeometry::GeometryCollection { geometries: _ } => {
                // We are inside a geometry collection but haven't opened a sub-geometry
                return Err(geozero::error::GeozeroError::Geometry(
                    "Coordinate without geometry type in GC?".into(),
                ));
            }
            CurrentGeometry::None => {
                return Err(geozero::error::GeozeroError::Geometry(
                    "Pushed coordinate but current_geom = None".into(),
                ));
                // return Err("Pushed coordinate but current_geom = None".into());
            }
        }
        Ok(())
    }
...
impl FeatureProcessor for GeoJsonBuilder {
    fn dataset_begin(&mut self, name: Option<&str>) -> Result<()> {
        // We can store the name in FeatureCollection's "foreign_members" if we want
        if let Some(name) = name {
            // Put e.g. "name": name in the .foreign_members
            let mut fm = self.collection.foreign_members.take().unwrap_or_default();
            fm.insert("name".to_string(), JsonValue::String(name.to_string()));
            self.collection.foreign_members = Some(fm);
        }
        Ok(())
    }

    fn dataset_end(&mut self) -> Result<()> {
        // We do nothing special here, features have presumably been appended
        Ok(())
    }
    ...
    fn feature_end(&mut self, _idx: u64) -> Result<()> {
        ...
        // Move the finished feature into the collection
        if let Some(feat) = self.current_feature.take() {
            self.collection.features.push(feat);
        }
        Ok(())
    }

    fn properties_begin(&mut self) -> Result<()> {
        // Clear our property buffer
        self.current_props = Map::new();
        Ok(())
    }

    fn properties_end(&mut self) -> Result<()> {
        // Assign the property buffer to the current feature
        if let Some(ref mut feat) = self.current_feature {
            feat.properties = Some(self.current_props.clone());
        }
        Ok(())
    }

    fn geometry_begin(&mut self) -> Result<()> {
        // We ensure that `self.current_geom` is reset.
        // In practice, geometry could appear multiple times if there's a geometry collection,
        // so we might need a more advanced approach with a geometry stack, but let's keep it simple here.
        self.current_geom = CurrentGeometry::None;
        Ok(())
    }

    fn geometry_end(&mut self) -> Result<()> {
        // We'll finalize the geometry in `feature_end` instead
        Ok(())
    }
}
impl PropertyProcessor for GeoJsonBuilder {
    fn property(
        ...
        // Insert into self.current_props
        let key = colname.to_string();
        let val = match colval {
            ...
            ColumnValue::String(s) | ColumnValue::DateTime(s) => {
                JsonValue::from(s.to_string())
            }
            ColumnValue::Json(s) => {
                // We can try to parse it as JSON
            ...
            ColumnValue::Binary(_v) => {
                // ignore or store base64
                JsonValue::Null
            }
        };
        self.current_props.insert(key, val);
        Ok(false)
    }
}
...
//         assert!(read_geojson(geojson.as_bytes(), &mut GeoJsonBuilder::new(&mut out)).is_ok());
...
//         let mut out: Vec<u8> = Vec::new();
//         let mut writer = GeoJsonBuilder::new(&mut out);
